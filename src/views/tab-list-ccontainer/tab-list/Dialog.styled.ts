import { css, html } from 'lit';
import { customElement, property } from 'lit/decorators.js';
import { styleMap } from 'lit/directives/style-map.js';
import { EventlessComponent } from '../../../core/Component.core';
import { IComponentEventType } from '../../../router';


const styledc = css`
  .dialog-container {
    position: absolute;
    top: 0;
    right: 0;
    transform: translateY(-50%);
    border-radius: 999px;
    display: flex;
    justify-content: space-evenly;
    align-items: center;
    background: linear-gradient(
      90deg,
      rgb(255, 255, 255) 50%,
      rgb(215, 255, 231) 50%
    );
    box-shadow: 0 2px 6px 1px rgba(0, 0, 0, 0.1);
    transition: ease 100ms;
  }

  .dialog-container svg {
    padding: 1px 4px;
    height: 15px;
  }

  .dialog-container svg:hover {
    background-color: rgba(0, 0, 0, 0.2);
  }

  .dialog-container .close {
    width: 13px;
    padding: 1px 4px 1px 6px;
    border-radius: 999px 0 0 999px;
  }

  .dialog-container .saved {
    width: 10px;
    padding: 1px 6px 1px 4px;
    border-radius: 0 999px 999px 0;
  }
`;

@customElement('app-tab-dialog-current')
export class TabListDialogCurrent extends EventlessComponent {
  @property()
  shouldShow = false;

  @property()
  win!: CurrentWindow

  private show;

  static styles = styledc;

  constructor() {
    super();

    this.show = {
      opacity: this.shouldShow ? '1' : '0',
      zIndex: this.shouldShow ? '999' : '0',
    } ;
  }

  closeHanlder() {
    this.sendToFront(IComponentEventType.USER_EVENT, {
      sender: this.constructor.name,
      data: {
        windowId: this.win.id,
        tabsLength: this.win.tabs.length,
        firstTabId: this.win.tabs[0].id
      },
      type: UsersEventType.CLOSE_WINDOW
    })
  }

  savedHandler() {
    this.sendToFront(IComponentEventType.USER_EVENT, {
      sender: this.constructor.name,
      data: {
        win: this.win
      },
      type: UsersEventType.SAVE_WINDOW
    })
  }

  render() {
    return html` <div
      class="dialog-container"
      style=${styleMap(this.show)}
    >
      <svg
        @click=${this.closeHanlder}
        class="close"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M18.3 5.70997C17.91 5.31997 17.28 5.31997 16.89 5.70997L12 10.59L7.10997 5.69997C6.71997 5.30997 6.08997 5.30997 5.69997 5.69997C5.30997 6.08997 5.30997 6.71997 5.69997 7.10997L10.59 12L5.69997 16.89C5.30997 17.28 5.30997 17.91 5.69997 18.3C6.08997 18.69 6.71997 18.69 7.10997 18.3L12 13.41L16.89 18.3C17.28 18.69 17.91 18.69 18.3 18.3C18.69 17.91 18.69 17.28 18.3 16.89L13.41 12L18.3 7.10997C18.68 6.72997 18.68 6.08997 18.3 5.70997Z"
          fill="black"
        />
      </svg>

      <svg
        @click=${this.savedHandler}
        class="saved"
        width="18"
        height="18"
        viewBox="0 0 18 18"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M14 0H2C0.89 0 0 0.9 0 2V16C0 17.1 0.89 18 2 18H16C17.1 18 18 17.1 18 16V4L14 0ZM16 16H2V2H13.17L16 4.83V16ZM9 9C7.34 9 6 10.34 6 12C6 13.66 7.34 15 9 15C10.66 15 12 13.66 12 12C12 10.34 10.66 9 9 9ZM3 3H12V7H3V3Z"
          fill="black"
        />
      </svg>
    </div>`;
  }
}

const styleds = css`
  .dialog-container-save-mode {
    position: absolute;
    top: 0;
    right: 0;
    transform: translateY(-50%);
    border-radius: 999px;
    display: flex;
    justify-content: center;
    align-items: center;
    align-items: center;
    background: #fff;
    box-shadow: 0 2px 6px 1px rgba(0, 0, 0, 0.1);
    transition: ease 100ms;
  }

  .dialog-container-save-mode .close-save-mode {
    width: 13px;
    height: 13px;
    padding: 2px;
    border-radius: 999px;
  }
  .dialog-container-save-mode svg:hover {
    background-color: rgba(0, 0, 0, 0.2);
  }
`;
@customElement('app-tab-dialog-saved')
export class TabListDialogSaved extends EventlessComponent {

  private show;

  @property()
  windowId!: number;

  @property()
  shouldShow = false;

  closeHandler() {
    this.sendToFront(IComponentEventType.USER_EVENT,{
      sender: this.constructor.name,
      data: { windowId: this.windowId },
      type: UsersEventType.DELETE_SAVED_WINDOW
    })
  }

  static styles = styleds;
  
  constructor() {
    super();

    this.show = {
      opacity: this.shouldShow ? '1' : '0',
      zIndex: this.shouldShow ? '999' : '0',
    } ;
  }

  render() {

    return html` 
    <div
      class="dialog-container-save-mode"
      style=${styleMap(this.show)}
    >
      <svg
        @click=${this.closeHandler}
        class="close-save-mode"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M18.3 5.70997C17.91 5.31997 17.28 5.31997 16.89 5.70997L12 10.59L7.10997 5.69997C6.71997 5.30997 6.08997 5.30997 5.69997 5.69997C5.30997 6.08997 5.30997 6.71997 5.69997 7.10997L10.59 12L5.69997 16.89C5.30997 17.28 5.30997 17.91 5.69997 18.3C6.08997 18.69 6.71997 18.69 7.10997 18.3L12 13.41L16.89 18.3C17.28 18.69 17.91 18.69 18.3 18.3C18.69 17.91 18.69 17.28 18.3 16.89L13.41 12L18.3 7.10997C18.68 6.72997 18.68 6.08997 18.3 5.70997Z"
          fill="black"
        />
      </svg>
    </div>`;
  }
}
